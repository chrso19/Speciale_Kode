...
def _apply_weather_forecast_noise(
    block_df: pd.DataFrame,
    rng: np.random.Generator | None = None,
) -> pd.DataFrame:
    """
    Corrupt WindSpeed and Radiation to simulate forecast inaccuracy.

    Accuracy tiers (noise = 1 - accuracy):
      Hours  1–72  → 97 % accurate → 3 % multiplicative noise
      Hours 73–120 → 92 % accurate → 8 % multiplicative noise
      Hours 121–168→ 77 % accurate → 23 % multiplicative noise

    Each value is multiplied by (1 + ε), where ε ~ Uniform(-noise, +noise),
    giving a symmetric relative error bounded by the tier's noise level.
    Values are clipped at 0 since both features are non-negative.
    """
    if rng is None:
        rng = np.random.default_rng()

    n = len(block_df)
    noise_levels = np.empty(n)

    for i in range(n):
        hour = i + 1          # 1-indexed position within the 168-hour block
        if hour <= 72:
            noise_levels[i] = 0.03
        elif hour <= 120:
            noise_levels[i] = 0.08
    ...
    for col in WEATHER_FEATURES:
        if col not in block_df.columns:
            continue
        values = block_df[col].to_numpy(dtype=float)
        epsilons = rng.uniform(-noise_levels, noise_levels)
        block_df[col] = np.clip(values * (1.0 + epsilons), 0.0, None)
...
def get_predictions(
    ...
    weather_noise : bool, default False
        If True, add tiered multiplicative noise to WindSpeed and Radiation to
        simulate forecast inaccuracy (hours 1-72: ±3%, 73-120: ±8%, 121-168: ±23%).
    rng : np.random.Generator | None, default None
        Random number generator for reproducible noise. If None, a new generator
        is created. Pass np.random.default_rng(seed) for reproducibility.
    ...
    while block_start_idx < len(horizon_hours):
        ...
        if block_no in cached_blocks:
            ...
            # Apply weather forecast noise after caching true feature values.
            if weather_noise:
