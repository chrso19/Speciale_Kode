        ...
        for timestamp in block_df["Time"]:
            ...
            if __name__ == "__main__":
                ...
                if y_scaler is not None:
                    y_pred = float(y_scaler.inverse_transform([[y_pred]])[0,0])

            y_pred_scaled = float(model.predict(X_window)[0])
            y_pred = y_pred_scaled if y_scaler is None else float(
                y_scaler.inverse_transform([[y_pred_scaled]])[0,0]
            )

            new_row[target_col] = y_pred_scaled
