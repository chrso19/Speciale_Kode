...
def get_predictions(
    ...
    while block_start_idx < len(horizon_hours):
        ...
        for step in range(n_block):
            ...
            if nan_mask.any():
                nan_cols = [col for col, idx in col_to_feat_idx.items() if nan_mask[idx]]
                warnings.warn(
                    f"NaN detected at block {block_no}, step {step} "
                    f"(time={block_hours[step]}) in features: {nan_cols}"
                )
            if nan_mask.any():
