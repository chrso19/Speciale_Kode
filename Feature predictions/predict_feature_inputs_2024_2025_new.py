...
OUTPUT_TEMPLATE = "feature_predictions_{zone}_2024-2025_new.csv"
...
def build_feature_blocks(
    ...
    if data.loc[data["Time"] < START].empty:
    ...
    while block_start_idx < len(horizon_hours):
        ...
        # Reset to true historical data before this block's start — matches get_predictions() behavior.
...
def main() -> None:
    ...
    for zone, data in zone_data.items():
        ...
        zone_predictions.to_csv(output_path, index=False, sep=";", decimal=".", date_format="%d-%m-%Y %H:%M:%S")
